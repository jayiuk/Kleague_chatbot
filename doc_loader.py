...
class document_loader():
    ...
    def url2doc(self):
        ...
        for doc in doc_list:
            ...
            replaced_texts = ["", "", "\n\ndate"]
            df = pd.DataFrame({'current' : texts, 'replaced' : replaced_texts})
            for item1, item2 in zip(df['current'], df['replaced']):
                doc.page_content = doc.page_content.replace(item1, item2)
        return doc_list
